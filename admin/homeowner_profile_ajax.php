...
if ($conn->connect_error) {
  ...
  echo '<div class="p-4"><div class="alert alert-danger mb-0">DB connection failed</div></div>';
  exit;
}
                    ...
                    <?php $i=1; while($m = $members->fetch_assoc()): ?>
                      <tr>
                        ...
                        <td class="fw-semibold">
                          <?= esc($m['first_name'] ?? '') ?> <?= esc($m['middle_name'] ?? '') ?> <?= esc($m['last_name'] ?? '') ?>
                        </td>
...
if ($admin_id_sess > 0) {
  ...
  $admin_phase = $a['phase'] ?? $admin_phase;
  $admin_role  = $a['role'] ?? $admin_role;
}

echo render_homeowner_profile_html($conn, $admin_role, $admin_phase, $id);
