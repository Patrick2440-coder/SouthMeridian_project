 ...
 */
function starts_with_custom($haystack, $needle) {
 ...
 * DB stores paths like "uploads/file.jpg"
 * Browser path from /admin should become "../uploads/file.jpg"
 */
function asset_path(string $path): string {
    ...
    if (starts_with_custom($p, '../') || starts_with_custom($p, '/')) return $p;

    if (starts_with_custom($p, 'uploads/')) return '../' . $p;
    if (starts_with_custom($p, 'admin/uploads/')) return '../uploads/' . basename($p);

    return '../' . ltrim($p, '/');
}

/**
 * Gets admin role + phase from session or DB fallback
 */
...
function render_homeowner_profile_html(mysqli $conn, string $admin_role, string $admin_phase, int $id): string {
    ...
    if ($displayId === '') {
        $displayId = phase_prefix($rowPhase) . (int)$homeowner['id'];
    }

    $latRaw = $homeowner['latitude'] ?? '';
    $lngRaw = $homeowner['longitude'] ?? '';

    $hasMap = ($latRaw !== '' && $lngRaw !== '' && is_numeric((string)$latRaw) && is_numeric((string)$lngRaw));
    $lat = $hasMap ? (float)$latRaw : null;
    $lng = $hasMap ? (float)$lngRaw : null;
    ...
    $validExt = strtolower(pathinfo(parse_url($validId, PHP_URL_PATH) ?? '', PATHINFO_EXTENSION));
    $proofExt = strtolower(pathinfo(parse_url($proof, PHP_URL_PATH) ?? '', PATHINFO_EXTENSION));

    $validIsPdf = ($validExt === 'pdf');
    $proofIsPdf = ($proofExt === 'pdf');
    ...
    <style>
      ...
      .profile-meta{
        padding-top:16px;padding-left:160px;display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;
      }
      .name{font-size:26px;font-weight:800;margin:0;line-height:1.15;}
      .subline{margin:6px 0 0;color:var(--muted);font-weight:600;}
      .pill{
        display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border-radius:999px;background:#f1f5f9;color:#0f172a;
    ...
    <div class="p-3 p-lg-4" style="background:var(--bg);">
      ...
      <div class="row g-4">
        <div class="col-lg-5">
          ...
          <div class="cardx mb-4">
            <div class="cardx-head">
              <h5 class="cardx-title">Pinned Location</h5>
            </div>
            <div class="cardx-body">
              <?php if ($hasMap): ?>
                <div class="kv">
                  <div class="k">Latitude</div>
                  <div class="v"><?= esc(number_format($lat, 6)) ?></div>

                  <div class="k">Longitude</div>
                  <div class="v"><?= esc(number_format($lng, 6)) ?></div>
                </div>
              <?php else: ?>
                <div class="text-muted fw-semibold">No pinned location found.</div>
              <?php endif; ?>
            </div>
          </div>
          ...
          <div class="cardx">
            <div class="cardx-head">
              <h5 class="cardx-title">Uploaded Valid ID and Proof Of Billing</h5>
            </div>
            <div class="cardx-body">
              <div class="row g-3">
                  ...
                  <div class="doc-card">
                    <?php if ($validId): ?>
                      ...
                      <?php else: ?>
                        ...
                        >
                        <div class="doc-thumb align-items-center justify-content-center text-muted fw-semibold" style="display:none;">
                          Unable to preview Valid ID
                        </div>
                      <?php endif; ?>
                    <?php else: ?>
                    ...
                    <div class="doc-meta">
                      <p class="doc-title mb-1">Valid ID Preview</p>
                      <?php if ($validId): ?>
                  ...
                  <div class="doc-card">
                    <?php if ($proof): ?>
                      ...
                      <?php else: ?>
                        ...
                        >
                        <div class="doc-thumb align-items-center justify-content-center text-muted fw-semibold" style="display:none;">
                          Unable to preview Proof of Billing
                        </div>
                      <?php endif; ?>
                    <?php else: ?>
                      <div class="doc-thumb d-flex align-items-center justify-content-center text-muted fw-semibold">
                    ...
                    <div class="doc-meta">
                      <p class="doc-title mb-1">Proof of Billing Preview</p>
                      <?php if ($proof): ?>
      ...
      <?php if ($status === 'pending'): ?>
        <div class="cardx mt-4">
          <div class="cardx-head">
            <h5 class="cardx-title">Registration Action</h5>
          </div>
          <div class="cardx-body">
            <div class="d-flex flex-wrap gap-2 justify-content-center">
              <button type="button" class="btn btn-success px-4 approveHomeowner" data-id="<?= (int)$homeowner['id'] ?>">
                Approve
              </button>
              <button type="button" class="btn btn-danger px-4 rejectHomeowner" data-id="<?= (int)$homeowner['id'] ?>">
                Reject
              </button>
            </div>
          </div>
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'homeowner_profile') {
    ...
    list($admin_role, $admin_phase) = get_admin_phase_role($conn);
...
// ============================================================
// If opened directly, redirect to approval page
// ============================================================
